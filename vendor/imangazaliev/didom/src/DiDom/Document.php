...
class Document
{
     ...
     */
    public function __construct($string = null, $isFile = false, $header = null, $encoding = 'UTF-8', $type = Document::TYPE_HTML)
    {
    ...
    protected function loadFile($filename)
    {
        ...
        try {
            if ($this->header != null) {
                $hdrs = array(
                  'http' =>array('header' => $this->header),
                );
                $context = stream_context_create($hdrs);
                $content = file_get_contents($filename, 0, $context);
            }else{
